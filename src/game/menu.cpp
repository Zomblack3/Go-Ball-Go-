...
namespace GoBallGo
{
	void menu(SCREENS& currentScreen, Sound& music, bool& isMusicOn, bool& wasMusicOn, Font font, bool& isGameplayMusicOn)
	{
		...
		Button tutorialButton = initButton(screenWidth / 2.0f, screenHeight - (screenHeight / 2.0f) + 100, BUTTONS_WIDTH, BUTTONS_HEIGHT, false);
		Button exitButton = initButton(screenWidth / 2.0f, screenHeight - (screenHeight / 2.0f) + 200, BUTTONS_WIDTH, BUTTONS_HEIGHT, false);
		...
		tutorialButton.texture = buttonsTexture;
		...
		tutorialButton.changeStateSound = buttonsChangeStateSound;
		...
		tutorialButton.x -= tutorialButton.w / 2.0f;
		exitButton.x -= exitButton.w / 2.0f;

		Vector2 mouse = { };

		while (!WindowShouldClose() && !playButton.isPressed && !creditsButton.isPressed && !tutorialButton.isPressed && !exitButton.isPressed)
		{
		...
		tutorialButton.isPressed = false;
...
namespace MenuStructure
{
	void update(SCREENS& currentScreen, Vector2& mouse, GoBallGo::Button& playButton, GoBallGo::Button& creditsButton, GoBallGo::Button& exitButton, GoBallGo::Button& tutorialButton, GoBallGo::Button& musicButton, Texture2D buttonOn, Texture2D buttonOff, bool& isMusicOn, bool& wasMusicOn, bool& isGameplayMusicOn, GoBallGo::Button& gameplayMusicButton)
	{
		...
		GoBallGo::mouseSelection(mouse, tutorialButton);
	...
	void draw(GoBallGo::Button& playButton, GoBallGo::Button& creditsButton, GoBallGo::Button& exitButton, GoBallGo::Button& tutorialButton, GoBallGo::Button& musicButton, Texture2D background, Texture2D logo, Font font, GoBallGo::Button gameplayMusicButton)
	{
		...
		std::string buttonsTexts[buttonsAmount] = { "START", "CREDITS", "TUTORIAL", "EXIT"};
		...
		Vector2 tutorialButtonTextPos = { (tutorialButton.x + tutorialButton.w / 2.0f) - (buttonsTextsLenghts[2] / 2.0f), tutorialButton.y + GoBallGo::normalFontSize - 5 };
		Vector2 exitButtonTextPos = { (exitButton.x + exitButton.w / 2.0f) - (buttonsTextsLenghts[3] / 2.0f), exitButton.y + GoBallGo::normalFontSize - 5 };
